...
class ChatResource(Resource):
    ...
    def generate_sse_from_json(self, response_data):
        """Generate SSE events from JSON response with the specified format"""
        # Send the response content
        yield self.format_sse_response(response_data)
        # Send the completion marker
        yield self.format_sse_response("", is_done=True)
    ...
    def post(self):
        ...
        if is_deep_research:
            ...
            for iteration in range(1, max_iterations + 1):
                ...
                if deep_research_response.get("status") != "success":
                    print(f"Deep research failed in iteration {iteration}. Error: {deep_research_response.get('error')}")
                    break
                ...
                if quality_check_result.get("status") != "success":
                    print(f"Quality check failed in iteration {iteration}. Error: {quality_check_result.get('error')}")
                    break
                
                # Skip to next iteration if no relevant keywords
                if not quality_check_result.get("top_relevance_keywords"):
                    print(f"No relevant keywords found in iteration {iteration}. Continuing.")
                    break
                ...
                if search_results_list:
                    ...
                    print(f"No search results found for keywords in iteration {iteration}.")
                    break
            ...
            final_prompt = f"""
            Based on the extensive research across {max_iterations} iterations:
            ...
            # Return the final comprehensive response
            return Response(
                self.ai_service.stream_chat(messages=data['messages']),
                # self.generate_sse_from_json(final_response.get("response", "")),
